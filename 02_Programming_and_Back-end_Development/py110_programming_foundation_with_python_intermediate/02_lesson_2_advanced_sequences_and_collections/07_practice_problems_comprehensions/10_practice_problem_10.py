...
def UUID():
    uuid = []
    for _ in range(8):
        uuid.append(str(random.choice(characters)))
    uuid.append('-')
...
'''could not figure out a method to do it with comprehension'''
...
def generate_uuid():
    ...
    for section in sections:
        chars = [random.choice(characters) for _ in range(section)]
